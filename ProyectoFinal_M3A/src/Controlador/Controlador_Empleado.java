...
//                throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    ...
    /*public void modificarEmpleado(){
        ...
        if(select!=-1){
            String rsp = String.valueOf(vistaE.getTablaEmpleados().getValueAt(select, 0).toString());
            List<ClaseEmpleado> tabla = modeloE.listaEmpleados();
            
            for(int j= 0; j<tabla.size(); j++){
                if(tabla.get(j).getId_empleado().equals(rsp)){
                    vistaE.getTxtIdEmpleado().setText(String.valueOf(tabla.get(j).getId_empleado()));
                    ...
                    vistaE.getCbSexo().setSelectedItem(tabla.get(j).getGenero());
                    vistaE.getTxtEdad().setText(String.valueOf(tabla.get(j).getEdad()));
                    vistaE.getTxtId_Usuario().setText(String.valueOf(tabla.get(j).getId_usuario()));
